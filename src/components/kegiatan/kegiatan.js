...
const Kegiatan = () => {
  ...
  const fetchData = async () => {
    ...
    // Sort the result based on tanggalPembuatan in descending order and take the first 4 items
      ...
      <div className='bg-[#677D6A]'>
        ...
        <div className="pl-10 pr-10 grid lg:grid-cols-4 md:grid-cols-2 pb-10 text-[#354b39] justify-center">
            {datas.map((data, index) => (
              <Link href={`/informasi/detailInformasi/${data.id}`}>
                <div key={index} className="bg-white shadow-lg rounded-lg m-5">
                    <img src={data.image} alt="kegiatan" layout="responsive" width={800} height={600} className="rounded-t-lg lg:h-[400px] h-[200px] md:h-[300]" />
                    <div className='p-5'>
